...
export function useUnitHandle(
  ...
  React.useEffect(() => {
    ...
    if (targetWindow && unit === 'viewport') {
      windowDimensionsRef.current.width = targetWindow.document.documentElement.getBoundingClientRect().width;
      windowDimensionsRef.current.height = targetWindow.document.documentElement.getBoundingClientRect().height;
