...
// } from './AccessibleMeetListsRenderer';
...
export const AccessibleMeetBase: React.FC<AccessibleMeetBaseProps> = ({
  variant,
}) => {
  ...
  return (
    <>
      {variant === 'gridsActiveOnlyCombinedNavigation' && (
        <h1>
          Variant B: Accessible Meet Using Grids Active Only Combined Navigation
        </h1>
      )}
      {variant === 'gridsActiveOnlyEntireRowNarration' && (
        <h1>
          Variant C: Accessible Meet Using Grids Active Only Entire Row
          Narration
        </h1>
      )}
      {variant === 'gridsActiveOnlyRowNavigation' && (
        <h1>
          Variant A: Accessible Meet Using Grids Active Only Row Navigation
        </h1>
      )}
      {variant === 'stitchedGridsRowNavigation' && (
        <h1>Accessible Meet Using Stitched Grids Row Navigation</h1>
      )}
      {variant === 'gridsRowNavigation' && (
        <h1>Accessible Meet Using Grids Row Navigation</h1>
      )}
      {variant === 'gridsFirstCellNavigation' && (
        <h1>Accessible Meet Using Grids First Cell Navigation</h1>
      )}
      {variant === 'gridsCellNavigation' && (
        <h1>Accessible Meet Using Grids Cell Navigation</h1>
      )}
      {variant === 'listsWithActions' && (
        <h1>Accessible Meet Using Lists With Actions</h1>
      )}
          ...
          <Menu>
            <MenuTrigger disableButtonEnhancement>
              {(triggerProps) => (
                <SplitButton
                  menuButton={{
                    ...triggerProps,
                    'aria-label': 'Schedule a different type meeting',
                  }}
                >
            ...
            <MenuPopover>
              <MenuList>
                ...
                <MenuItem>
                  Webinar Host interactive event with registration
                </MenuItem>
                <MenuItem>Town hall Produce event for large audience</MenuItem>
                <MenuItem>
                  Virtual appointment Guests join on web and enter via tailored
                  lobby
                </MenuItem>
                <MenuItem>
                  Controlled-content meeting Apply extra meeting content
                  controls
                </MenuItem>
