...
const getRegisteredTabRect = (
  registeredTabs: Record<string, TabRegisterData>,
  value?: TabValue
) => {
  const element =
    value !== undefined && value !== null
      ? registeredTabs[JSON.stringify(value)]?.ref.current
      : undefined;
 ...
 */
export const useTabAnimatedIndicatorStyles_unstable = (
  state: TabState
): TabState => {
  ...
  const getRegisteredTabs = useTabListContext_unstable(
    (ctx) => ctx.getRegisteredTabs
  );
  ...
  if (selected) {
    const { previousSelectedValue, selectedValue, registeredTabs } =
      getRegisteredTabs();

    if (previousSelectedValue && lastAnimatedFrom !== previousSelectedValue) {
      const previousSelectedTabRect = getRegisteredTabRect(
        registeredTabs,
        previousSelectedValue
      );
      const selectedTabRect = getRegisteredTabRect(
        registeredTabs,
        selectedValue
      );
  ...
  state.root.className = mergeClasses(
    ...
    selected &&
      (vertical
        ? activeIndicatorStyles.vertical
        : activeIndicatorStyles.horizontal)
  );
